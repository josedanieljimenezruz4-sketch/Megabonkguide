...
<div class="container mx-auto px-4 py-8">
    <div class="mb-10">
        ...
        <p class="text-gray-400">Administra, edita o elimina contenido generado por los usuarios (Builds y Tier Lists).</p>
    </div>
    ...
    <div class="flex flex-col gap-8">
        ...
        <!-- TIER LISTS -->
        <div class="glass-card p-6 rounded-2xl mb-8 bg-gray-900/40 backdrop-blur-xl border border-white/10 shadow-[0_8px_30px_rgba(0,0,0,0.5)]">
            <h2 class="text-xl text-purple-400 font-bold mb-6 flex items-center gap-2 border-b border-white/5 pb-4">📊 Moderación de Tier Lists</h2>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-black/60 text-xs uppercase tracking-wider text-purple-400">
                        <tr>
                            ...
                            <th class="p-4 font-semibold">Votos</th>
                            <th class="p-4 text-center rounded-tr-lg font-semibold">Acciones</th>
                        </tr>
                    </thead>
                    <tbody class="text-sm text-gray-300">
                        @forelse($tierlists as $tl)
                            <tr class="border-b border-white/5 hover:bg-white/5 transition-colors group">
                                <td class="p-4 text-white font-medium">{{ $tl->user->username ?? $tl->user->name ?? 'Anónimo' }}</td>
                                <td class="p-4 font-semibold">{{ \Illuminate\Support\Str::limit($tl->title, 30) }}</td>
                                <td class="p-4"><span class="bg-white/10 px-3 py-1 rounded-full text-xs uppercase border border-white/10">{{ ucfirst($tl->category) }}</span></td>
                                <td class="p-4"><strong class="text-red-400">{{ $tl->upvotes }} 🔥</strong></td>
                                <td class="p-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.moderation.tierlists.destroy', $tl->id) }}" method="POST" onsubmit="return confirm('¿Seguro que deseas eliminar esta Tier List comunitaria?');">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr><td colspan="5" class="p-10 text-center text-gray-500 italic">No hay tier lists registradas.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div class="mt-6 flex justify-center">{{ $tierlists->links() }}</div>
        </div>
